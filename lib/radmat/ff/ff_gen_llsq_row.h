...
namespace radmat
{
  ...
  struct SemblePInv {
    ...
    typedef std::pair<SEMBLE::SembleVector<double> ,  SEMBLE::SembleVector<double> > PInv_t;

    // return a vec
    SEMBLE::SembleVector<double> p_f(void)  {return p.first;}
    // return a ref to this vec
    const SEMBLE::SembleVector<double>& pf(void) const {return p.first;}
    SEMBLE::SembleVector<double>& pf(void) {return p.first;}

    // return a vec
    SEMBLE::SembleVector<double> p_i(void) {return p.second;}
    // return a ref to this vec
    const SEMBLE::SembleVector<double>& pi(void) const {return p.second;}
    SEMBLE::SembleVector<double>& pi(void) {return p.second;}



    ENSEM::EnsemReal Q2(void)
    {
      SEMBLE::SembleVector<double> q(p_i() - p_f()); 
  ...
  SemblePInv makeMomInvariants(const EnsemReal &E_f, 
      const EnsemReal &E_i,
      const Array<int> &p_f,
      const Array<int> &p_i,
  ...
  template<typename T>
    struct ffKinematicFactors_t
    {
      ...
      KinematicFactorMatrix genFactors(const SemblePInv &moms)
      {
        SEMBLE::SembleVector<double> p_f(moms.pf()), p_i(moms.pi());
