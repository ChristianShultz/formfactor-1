...
namespace radmat
{
  ...
  {  
    ...
    TensorImplBase* get(const pFacKey &k);                           //! get the data for a key
    TensorImplBase* operator()(const pFacKey &k) {return get(k);}    //! get the data for a key
  ...
  */
  ...
  private:
    ...
    Tensor<std::complex<double> , J >* operator()(void)
    {
      ...
      Tensor<std::complex<double> , J> ret;
      ...
      for(short h = -(J-1); h < J; ++h)
	{
	  ...
	  for(short h1 = -1; h1 < 2; ++h1)
	      ...
	      if(cg != 0.)
		{
		  ...
		  ret+= downcastAndDeref<Tensor<std::complex<double> , J - 1>, TensorImplBase >(factory->get(k_Jm1_h))
		    ^ downcastAndDeref<Tensor<std::complex<double> , 1>, TensorImplBase >(factory->get(k_1_h1))
		    * cg;
		}
	    }
	}
  ...
  */
  ...
  struct Coupler<1>
  {
     ...
     */
    Tensor<std::complex<double> , 1> * operator()(void)
    {
      pFacKey k_z(k);
      ...
      Tensor<std::complex<double> , 1> eps_z;

      if(pFacInv::pFacInvIsRegistered(k_z))  // check for eps_z -- do we have the k_z data, skip some work if we do 
	{
	  eps_z = downcastAndDeref<Tensor<std::complex<double> , 1> , TensorImplBase>(pFacInv::pFacInvGetData(k_z));
	}
	...
	return eps_z.clone();
      ...
      pFacInv::pFacInvRegister(k_z,eps_z.clone());
      ...
      R = breit::rodRotMat(pz,p);

      // set up the polarization vector along p 
      Tensor<std::complex<double>,1> eps;
      eps = eps_z;
