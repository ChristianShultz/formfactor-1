...
namespace radmat
{
  ...
  */
  ...
  struct Tensor : public PrimitiveTensor<T, N>
  {
    ...
    /*  TO DO -- test on qcd10i1 and qcd10i2, abs by itself returns an int.
        the cmath version is overloaded to do what one would expect,
	may have to change the functor definition and specialize
        ...
        int main ()
        {
        std::cout << "The absolute value of 3.1416 is " << std::abs (3.1416) << std::endl;
        std::cout << "The absolute value of -10.6 is " << std::abs (-10.6) << std::endl;
        return 0;
        }

        my-macbook-pro:tensor shultz$ ./a.out
    ...
    */
    ...
    void operator-=(const Tensor<T, N> &o);
    ...
    */
    ...
    //! overload contract to inner_product for two rank1 tensors, checks raised/lowered
    template<typename U, typename UU>
    friend
    typename Promote<U, UU>::Type_t
    contract(const Tensor<U, 1> &a,
	     const Tensor<UU, 1> &b,
	     const idx_t aa = 0 ,
	     const idx_t bb = 0);
    ...
    friend
    Tensor < typename Promote<typename Promote<U, UU>::Type_t, UUU>::Type_t, M + MM - 2 >
    contract(const Tensor<U, M> &A,
	     const Tensor<UU, MM> &B,
  ...
  // the stl is magical
  template<typename T, idx_t N>
  void Tensor<T, N>::operator*=(const T &factor)
  {
  ...
  template<typename T, idx_t N>
  void Tensor<T, N>::operator-=(const Tensor<T, N> &o)
  {
...
((typename Promote<T,U>::Type)A.getElem(a) 
  ...
  template<typename T, typename U>
  typename Promote<T,U>::Type_t contract(const Tensor<T, 1> &a, const Tensor<U, 1> &b, const idx_t aa , const idx_t bb)
  {
    idx_t sz = a.size();
