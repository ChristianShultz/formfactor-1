...
namespace radmat
{
  ...
  template<>
    struct genPolTens<1>
    {
      ...
      Tensor<std::complex<double> , 1> get(const double E, const short hel, const double mom_factor)
      {
        ...
        h_map->insert(map_t::value_type(pKey_t(1,hel),make(E,hel,mom_factor).clone()));
      ...
      Tensor<std::complex<double>,1> make(const double E, const short hel, const double mom_factor)
      {
        ...
        {
          in_bounds = true;
          double m_p = mom_factor*sqrt( (*h_mom)[0] * (*h_mom)[0] 
              +  (*h_mom)[1] * (*h_mom)[1] 
              +  (*h_mom)[2] * (*h_mom)[2] );
          ...
          if(!!!(m_m > 1e-14))
          {
            ...
            std::cout << "px = " << (*h_mom)[0]
              << "py = " << (*h_mom)[1]
              << "pz = " << (*h_mom)[2] << std::endl;
            std::cout << "mom_fac = " << mom_factor << std::endl;
        ...
        // R is an active rotation -- these rotate like helicity creation operators or equivalently helicity states
        return *R*eps; 
      }
  ...
  template<idx_t J>
    struct genPolTens
    {
      ...
      void make(const double E, const short hel, const double mom_factor)
      {
        ...
        for(short big_h = -(J-1); big_h < J ; ++big_h)
          for(short small_h = -1; small_h < 2; ++small_h)
          {
            ...
            if(factor != 0.)
              ptensor += (factor * (Jm.get(E,big_h,mom_factor) ^ J1.get(E,small_h,mom_factor)));
          }
  ...
  template<>
    struct genPolTens3D<1>
    {
      ...
      genPolTens3D(const mom_t &_p)
        : h_map(new map_t()) , R(genRotationMatrix3D(_p).clone()) , h_mom(new mom_t(_p))
      { }
      ...
      //!  do not use!
      genPolTens3D(map_handle &_h_map, R_ij _R, mom_handle & _h_mom )
        : h_map(_h_map) , R(_R) , h_mom(_h_mom)
      { }
      ...
      {
        Tensor<std::complex<double> ,1 > eps((TensorShape<1>())[3],0.);
        bool in_bounds(false);
        if(hel == 1)
        {
          in_bounds = true;
          eps[0] = -1./sqrt(2.);
          eps[1] = std::complex<double>(0.,-1./sqrt(2.));
        }
        if(hel == -1)
        {
          in_bounds = true;
  ...
  template<idx_t J>
    struct genPolTens3D
    {
      ...
      {
        genPolTens3D< J - 1 > Jm(h_map, R, h_mom);
        genPolTens3D<1> J1(h_map, R, h_mom);
        double factor;
