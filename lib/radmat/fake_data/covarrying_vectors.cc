...
namespace radmat
{
  ...
  namespace
  {
    ...
    template<>
      double randomPhase<double>(void)
      {
        if(itpp::randb()) 
          return -1.;
        return 1.;
    ...
    template<>
      std::complex<double> randomPhase<std::complex<double> >(void)
      {
        double rn = itpp::randu()*6.28319; // (0,2pi) 
        return std::complex<double>(cos(rn),sin(rn)); //exp(iphi) 
  ...
  template<>
    itpp::Mat<double> corMat::genMat<double>(const int rank) const
    {
      ...
      //    std::cout <<  0.5*(foo + itpp::hermitian_transpose(foo)) << std::endl;
