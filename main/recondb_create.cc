 ...
 * Last Modified : Tue Dec 11 08:33:16 2012
...
int main(int argc, char *argv[])
{
  ...
  else if(inikeys.dbInputType == "dbnew")
  {
    std::cout << "WARNING: this is hackey and shouldnt be used in real life."
      << "\nwe're trying to fake up this database using dbnew, should be using redstar corrs" << std::endl;
    // get the data into a useful form
    std::cout << "reading ops phases" << std::endl;
    std::map<std::string, int> opmap = readOpsPhases();
    std::string fake_ops("fake_ops_list.xml");
    XMLArray::Array<std::string> fake_ops_list; 
    fake_ops_list.resize(1);
    fake_ops_list[0] = fake_ops; 
    std::cout << "reading " << fake_ops << std::endl; 
    std::map<std::string, Hadron::KeyHadronNPartIrrep_t> keymap = readOpsXML(fake_ops_list, namelist);
    std::cout << "chomping" << std::endl;
    std::vector<stupid_datum> data = chomp(namelist, keymap, opmap , t0);
    std::vector<stupid_datum>::const_iterator it;
